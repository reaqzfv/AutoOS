...
public static class BrowserStage
{
    public static async Task Run()
    {
        ...
        var actions = new List<(string Title, Func<Task> Action, Func<bool> Condition)>
        {
            ...
            ("Optimizing Zen settings", async () => await ProcessActions.RunCustom(async () => await Task.Run(() => { string installDir = @"C:\Program Files\Zen Twilight"; string autoconfigPath = Path.Combine(installDir, "defaults", "pref", "autoconfig.js"); string autoconfigContent = "pref(\"general.config.filename\", \"zen.cfg\");\npref(\"general.config.obscure_value\", 0);"; File.WriteAllText(autoconfigPath, autoconfigContent); })), () => Zen == true),
            ("Optimizing Zen settings", async () => await ProcessActions.RunCustom(async () => await Task.Run(() => { string installDir = @"C:\Program Files\Zen Twilight"; string zenConfigPath = Path.Combine(installDir, "zen.cfg"); string zenConfigContent = "\ndefaultPref(\"app.shield.optoutstudies.enabled\", false);\ndefaultPref(\"datareporting.healthreport.uploadEnabled\", false);\ndefaultPref(\"browser.newtabpage.activity-stream.feeds.section.topstories\", false);\ndefaultPref(\"browser.newtabpage.activity-stream.feeds.topsites\", false);\ndefaultPref(\"dom.security.https_only_mode\", true);\ndefaultPref(\"full-screen-api.transition-duration.enter\", \"0 0\");\ndefaultPref(\"full-screen-api.transition-duration.leave\", \"0 0\");\ndefaultPref(\"full-screen-api.warning.timeout\", 0);\ndefaultPref(\"nglayout.enable_drag_images\", false);\ndefaultPref(\"reader.parse-on-load.enabled\", false);\ndefaultPref(\"browser.tabs.firefox-view\", false);\ndefaultPref(\"browser.tabs.tabmanager.enabled\", false);\nlockPref(\"browser.newtabpage.activity-stream.asrouter.userprefs.cfr.addons\", false);\nlockPref(\"browser.newtabpage.activity-stream.asrouter.userprefs.cfr.features\", false);"; File.WriteAllText(zenConfigPath, zenConfigContent); })), () => Zen == true),
            ("Optimizing Zen settings", async () => await ProcessActions.RunCustom(async () => await Task.Run(() => { string installDir = @"C:\Program Files\Zen Twilight"; string policiesPath = Path.Combine(installDir, "distribution", "policies.json"); var policiesContent = new { policies = new { DisableAppUpdate = true, OverrideFirstRunPage = "" } }; File.WriteAllText(policiesPath, JsonSerializer.Serialize(policiesContent, new JsonSerializerOptions { WriteIndented = true })); })), () => Zen == true),

            // download arkenfox user.js
            ("Downloading Arkenfox user.js", async () => await ProcessActions.RunDownload("https://github.com/arkenfox/user.js/raw/refs/heads/master/user.js", @"C:\Program Files\Zen Browser", "user.js"), () => Zen == true),
