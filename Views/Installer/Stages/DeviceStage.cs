...
public static class DeviceStage
{
    ...
    public static async Task Run()
    {
        ...
        var actions = new List<(string Title, Func<Task> Action, Func<bool> Condition)>
        {
            ...
            ("Disabling DMA remapping", async () => await ProcessActions.RunNsudo("TrustedInstaller", @"cmd /c for %a in (DmaRemappingCompatible) do for /f ""delims="" %b in ('reg query ""HKLM\SYSTEM\CurrentControlSet\Services"" /s /f ""%a"" ^| findstr ""HKEY""') do reg add ""%b"" /v ""%a"" /t REG_DWORD /d 0 /f"), null),
            ...
            ("Disabling XHCI Interrupt Moderation (IMOD)", async () => await ProcessActions.RunPowerShellScript("imod.ps1", $"-disable \"{Path.Combine(PathHelper.GetAppDataFolderPath(), "Chiptool", "chiptool.exe")}\""), () => IMOD == false),
