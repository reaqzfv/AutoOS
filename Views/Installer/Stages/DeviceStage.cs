...
public static class DeviceStage
{
    public static async Task Run()
    {
        ...
        var actions = new List<(string Title, Func<Task> Action, Func<bool> Condition)>
        {
            ...
            ("Enable write caching on all drives", async () => await ProcessActions.RunNsudo("TrustedInstaller", @"cmd /c for /f ""delims="" %k in ('reg query HKLM\SYSTEM\CurrentControlSet\Enum /f ""{4d36e967-e325-11ce-bfc1-08002be10318}"" /s /d ^| find ""HKEY""') do reg add ""%k\Device Parameters\Disk"" /v UserWriteCacheSetting /t REG_DWORD /d 1 /f"), null),

            // disable write-cache buffer flushing on all drives
            ("Disable write-cache buffer flushing on all drives", async () => await ProcessActions.RunNsudo("TrustedInstaller", @"cmd /c for /f ""delims="" %k in ('reg query HKLM\SYSTEM\CurrentControlSet\Enum /f ""{4d36e967-e325-11ce-bfc1-08002be10318}"" /s /d ^| find ""HKEY""') do reg add ""%k\Device Parameters\Disk"" /v CacheIsPowerProtected /t REG_DWORD /d 1 /f"), null),
